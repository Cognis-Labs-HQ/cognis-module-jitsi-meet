...
export function registerApiRoutes(router, ctx) {
  ...
  const resolveShareGuestMeetingAccess = async ({
    ...
    const resolveGuestAccess = ctx.getCapability('share:resolveGuestAccess');
    ...
    return {
      isGuest:
        access?.shareGuest === true || legacyMeetingAccess?.shareGuest === true,
      allowed:
        access?.authorized === true || legacyMeetingAccess?.authorized === true,
      tokenRecord: null,
    };
  };
  const resolveShareUserAccess = ctx.getCapability('share:resolveUserAccess');
  const deleteResourceShares = ctx.getCapability('share:deleteResourceShares');
  ...
  if (!dbExecutor || !profileStore) {
    ...
    router.get('/api/v1/modules/jitsi-meet/participants', async (_req, res) => {
  ...
  async function dispatchMeetingNotifications(
    ...
    const organizerProfile = organizerUsername
      ? await profileStore
          ...
          .catch((error) => logApiFallback(error, 'index_fallback', null))
      : null;
    ...
    for (const recipientUsername of candidateRecipients) {
      ...
      if (!recipientProfile?.accountId) {
        log?.('error', 'Failed to resolve meeting notification recipient.', {
          component: 'jitsi-meet-module',
      ...
      normalizedRecipients.push({
        ...
        username:
          normalizeHandleKey(recipientProfile.handle) || recipientUsername,
      });
    }
  ...
  async function resolveModeratorUsernames(meeting, participantUsernames) {
    const normalizedParticipants = Array.from(
      new Set(
        (Array.isArray(participantUsernames) ? participantUsernames : [])
          .map((username) => normalizeHandleKey(username))
          .filter(Boolean),
      ),
    );
