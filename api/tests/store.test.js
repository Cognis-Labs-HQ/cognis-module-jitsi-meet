...
function createMockJitsiDb({
  ...
  return {
    ...
    async executeCommand(command) {
      if (command.option === 'UPDATE' && command.table === 'jitsi_meetings') {
      ...
      if (
        ...
        const participantRow = storedParticipantRows.find(
          (row) => row.meeting_id === meetingId && row.username === username,
        );
      ...
      if (command.option === 'INSERT' && command.table === 'jitsi_meetings') {
