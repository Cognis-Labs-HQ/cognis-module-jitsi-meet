...
function createProfileStore() {
  ...
  return {
    async getProfile(accountId) {
      const handle =
        handleByAccountId[String(accountId ?? '')] ?? String(accountId ?? '');
      return {
    ...
    async getProfileByHandle(handle) {
      ...
      return {
        accountId: accountIdByHandle[normalizedHandle] ?? normalizedHandle,
        handle: normalizedHandle,
        displayName:
          normalizedHandle === 'alice' ? 'Alice Example' : normalizedHandle,
      };
    },
  };
}
  ...
  async executeCommand(command) {
    if (command.table === 'jitsi_meetings') {
      if (
        ...
        command.where.some(
          (condition) =>
            condition.column === 'id' && condition.value !== this.meetingRow.id,
        )
      ) {
        return { rows: [] };
      }
...
test('jitsi share hooks let participants mint and revoke meeting shares', async () => {
  ...
  assert.equal(mintResult.stageResults['authorize-minter'][0].authorized, true);
  assert.equal(
...
test('jitsi share hooks flag direct access for the meeting owner and participants', async () => {
  ...
  assert.equal(ownerResult.stageResults['check-access'][0].directAccess, true);
...
test('meeting share routes let participants manage shares and reject outsiders', async () => {
  ...
  const handlers = createRouterHarness({
    ...
    runFlow: async (flowId, payload) => {
      ...
      if (flowId === 'mint-share-token') {
        return {
          stageResults: {
            'issue-token': [{ minted: true, shareRecord: { id: 'share-3' } }],
          },
        };
      }
      return {
  ...
  for (const request of [
    {
      handler: outsiderHandlers.get('GET /api/v1/modules/jitsi-meet/share'),
      method: 'GET',
      url: '/api/v1/modules/jitsi-meet/share?meetingId=meeting-1',
    },
    {
      handler: outsiderHandlers.get('POST /api/v1/modules/jitsi-meet/share'),
      method: 'POST',
      url: '/api/v1/modules/jitsi-meet/share',
...
test('meeting share approval targets only include currently present participants', async () => {
  ...
  assert.deepEqual(result.stageResults['resolve-targets'][0].targetAccountIds, [
    'bob-account',
  ]);
});
