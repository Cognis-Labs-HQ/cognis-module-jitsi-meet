...
export function registerMeetingRoutes({
  ...
  router.get(
    ...
    async (req, res) => {
      ...
      if (
        !['all', '1', '0', 'true', 'false', 'yes', 'no'].includes(meetingFilter)
      ) {
        sendError(
          res,
          400,
      ...
      if (startAtQuery || endAtQuery) {
        ...
        if (!windowStart || !windowEnd || windowEnd <= windowStart) {
          sendError(res, 400, 'bad_request', 'Invalid startAt/endAt range.');
          return;
        }
      } else if (dateQuery) {
      ...
      const filteredRows = eventRows
        .filter((event) => {
          ...
          const hasMeeting = Boolean(String(event.meetingUrl ?? '').trim());
          if (['1', 'true', 'yes'].includes(meetingFilter) && !hasMeeting) {
            return false;
          }
          if (['0', 'false', 'no'].includes(meetingFilter) && hasMeeting) {
            return false;
          }
          ...
          if (
            windowStart &&
            windowEnd &&
            !overlapsWindow(eventStart, eventEnd, windowStart, windowEnd)
          ) {
            return false;
          }
      ...
      for (const activeMeeting of activeMeetings) {
        ...
        const startedByProfile = startedByUsername
          ? await profileStore
              ...
              .catch((error) =>
                logApiFallback(error, 'meetings_routes_fallback', null),
              )
          : null;
        ...
        const activeParticipantProfiles =
          activeParticipantHandles.length > 0
            ...
            : [];
        const activeParticipants = activeParticipantProfiles.map((profile) => ({
        ...
        visibleMeetings.push({
          ...
          activeParticipantCount: Number(activeMeeting.activeParticipantCount),
      ...
      if (shareGuestAccess.isGuest) {
        ...
        const participants =
          typeof filterUsernamesForGuestVisibility === 'function'
            ? await filterUsernamesForGuestVisibility(rawParticipants)
            : rawParticipants;
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const liveness = await checkHttpLiveness(resolved.meeting.meetingUrl, {
        timeoutMs: LIVELINESS_TIMEOUT_MS,
      });
      sendJson(res, 200, {
        data: liveness,
        ...
        sendJson(res, 200, {
          data: {
            ...
            activeParticipants: await (async () => {
              ...
              return typeof filterUsernamesForGuestVisibility === 'function'
                ? filterUsernamesForGuestVisibility(activeUsernames)
                : activeUsernames;
            })(),
            sessionActive: true,
          },
