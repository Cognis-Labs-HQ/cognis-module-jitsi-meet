    ...
    async (req, res) => {
      ...
      if (typeof resolveGroupChat === 'function') {
        ...
        chatRoom = await resolveGroupChat({
          ...
          logApiFallback(error, 'meeting_lifecycle_routes_fallback', null),
        );
      }
      ...
      const activeParticipants = await Promise.all(
        activeUsernames.map(async (username) => {
          ...
          return {
            ...
            displayName: profile?.displayName ?? profile?.handle ?? username,
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (shareGuestAccess.isGuest) {
        ...
        const guestPresenceUsername = resolveShareGuestPresenceUsername(claims);
        ...
        data: {
          ...
          meetingClosed:
            ...
            (previousSessionPresence?.active && !nextPresenceActive
              ? (await store.getMeetingState(resolved.meeting.id)).endedAt !==
                null
              : false),
        },
      });
