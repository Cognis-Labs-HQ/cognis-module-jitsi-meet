...
export function createChatHandlers({
  ...
  async function refreshNativeChat() {
    ...
    const ordered = Array.isArray(payload?.data)
      ? payload.data
          ...
          .filter(
            (message) =>
              message?.contentType !== 'application/vnd.cognis.room-event+json',
          )
      : [];
  ...
  async function activatePrivateChatForParticipant(username) {
    ...
    if (!normalizedUsername) {
      ...
      showToast(i18n.t('module.jitsi_meet.chat.private_open_unavailable'), {
        variant: 'warning',
      });
      return;
    }
    ...
    if (!roomId) {
      console.error('[jitsi-meet] private chat room response missing room id', {
        operation: 'open_private_chat',
        targetUsername: normalizedUsername,
        payload,
      });
