...
export async function mount(
  ...
  function collectMeetingSearchGroups() {
    const meetings = [
      ...(Array.isArray(state.activeMeetings) ? state.activeMeetings : []),
  ...
  function bindInteractiveHandlers() {
    ...
    const findButton = container.querySelector('#jitsi-find-participants-btn');
      ...
      findButton.addEventListener(
        ...
        () => {
          ...
          openSearchPopup({
            ...
            category: 'user',
            ariaLabel: i18n.t('module.jitsi_meet.participants.search'),
            noResultsText: i18n.t('module.jitsi_meet.participants.none'),
            confirmLabel: i18n.t('module.jitsi_meet.participants.add_selected'),
            multiSelect: true,
    ...
    container.addEventListener(
      ...
      (event) => {
        ...
        const avatar = event.target.closest('[draggable][data-username]');
    ...
    if (chatThread instanceof HTMLElement) {
      chatThread.addEventListener(
        ...
        async (clickEvent) => {
          ...
          if (reactionButton.classList.contains('messages-reaction-add-btn')) {
    ...
    if (authButton instanceof HTMLButtonElement) {
      authButton.addEventListener(
        ...
        async () => {
          ...
          window.open(
            buildMeetingJoinUrl(state.meeting.meetingUrl, state.currentProfile),
            '_blank',
            'noopener,noreferrer',
          );
        },
    ...
    const leaveAloneButton = container.querySelector('#jitsi-leave-alone-btn');
      ...
      leaveAloneButton.addEventListener(
        ...
        async () => {
          ...
          await resetMeetingState({
            overlayMessageKey: 'module.jitsi_meet.overlay.meeting_left',
          });
        },
        { signal: bindSignal },
      );
    ...
    if (
      ...
      chatForm.addEventListener(
        ...
        async (event) => {
          ...
          if (!roomKey) return;
          const encrypted = await encryptChatMessage(messageText, roomKey);
  ...
  state.allParticipants = allParticipants
    .map((entry) => ({
      ...
      avatarKey: typeof entry?.avatarKey === 'string' ? entry.avatarKey : null,
    }))
