...
export function createPreflightHandlers({
  ...
  function renderParticipants() {
    const availablePool = root.querySelector('#jitsi-available-participants');
  ...
  async function loadMeetingState() {
    ...
    if (payload?.data?.sessionActive === false) {
      await callbacks.resetMeetingState({
        overlayMessageKey: 'module.jitsi_meet.overlay.reclaimed_elsewhere',
        toastMessageKey: 'module.jitsi_meet.overlay.reclaimed_elsewhere',
  ...
  async function keepPresenceAlive(active = true, { terminated = false } = {}) {
  ...
  function getLocalParticipantInfo(apiInstance) {
    if (!apiInstance || typeof apiInstance.getParticipantsInfo !== 'function') {
      return null;
    }
  ...
  function currentUserIsJitsiModerator(apiInstance) {
    ...
    if (
      ...
      try {
        ...
        console.warn('[jitsi-meet] failed to check Jitsi moderator status', {
    ...
    return (
      getParticipantRole(getLocalParticipantInfo(apiInstance)) === 'moderator'
    );
  }
  ...
  function recoverMeetingSessionAfterComposerRender() {
    ...
    try {
      ...
      console.warn('[jitsi-meet] failed to dispose stale meeting session', {
        component: 'jitsi-meet-module',
    ...
    void callbacks
      .joinMeeting()
      .catch((error) => {
        console.error('[jitsi-meet] failed to recover meeting session', {
          component: 'jitsi-meet-module',
