...
export function createEmbedHandlers({
  ...
  async function openMeetingEmbed() {
    ...
    const meetingKeyring = createKeyringScope?.(i18n.t('ui.reuse.meetings'));
    ...
    const meetingProcess = i18n
      ...
      .replace('{{meeting}}', state.meeting.meetingName || state.meeting.id);
    ...
    const promptForCurrentMeetingPassword = async ({ invalid }) => {
      ...
      const result = await openPopup({
        ...
        onOpen: (overlay) => {
          passwordInput = overlay.querySelector('#jitsi-keyring-password');
          passwordInput?.focus();
        },
        onAction: (actionId) =>
    ...
    const handleMeetingLeft = () => {
      ...
      void callbacks.handleMeetingExit({
        fallbackOverlayMessageKey: 'module.jitsi_meet.overlay.meeting_left',
    ...
    const handleMeetingTerminated = () => {
      if (state.jitsiApi !== apiInstance) return;
      void callbacks.handleMeetingExit({
        fallbackOverlayMessageKey: 'module.jitsi_meet.overlay.meeting_closed',
    ...
    apiInstance.addEventListener('videoConferenceJoined', (event) => {
      ...
      state.jitsiModerator = callbacks.currentUserIsJitsiModerator(apiInstance);
    ...
    apiInstance.addEventListener('passwordRequired', async () => {
      ...
      if (submittedStoredPassword) {
        const replacement = await meetingKeyring.resolve(meetingKeyringId, {
          validate: () => false,
          prompt: promptForCurrentMeetingPassword,
          request: {
            action: i18n.t('module.jitsi_meet.keyring_request_action_update'),
            process: meetingProcess,
          },
          metadata: {
            label: state.meeting.meetingName || i18n.t('ui.reuse.meeting'),
