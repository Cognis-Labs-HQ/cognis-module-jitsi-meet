...
export async function loadMessageUiResources() {
  try {
    const response = await apiFetch('/api/v1/modules/jitsi-meet/ui-resources');
    if (!response.ok) {
      console.warn(
...
export async function loadMessageReactionsController(
  ...
  try {
    ...
    if (typeof moduleExports?.createMessageReactionsController !== 'function') {
      return null;
    }
    const reactionsController = moduleExports.createMessageReactionsController({
      i18n,
      onReactionUpdated,
    });
...
async function fetchShareGuestProfile() {
  ...
  try {
    ...
    console.error('[jitsi-meet] failed to load share guest profile support', {
      component: 'jitsi-meet-module',
