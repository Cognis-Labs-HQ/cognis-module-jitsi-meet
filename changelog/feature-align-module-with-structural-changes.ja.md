...
マニフェストは、ホスト所有のミーティング Capability と Flow に必要な信頼済み特権連携を明示し、Bootstrap を唯一のランタイム連携エントリポイントとして使用するようになりました。さらに、バージョン 1.5.210 と宣言済みの全ファイルダイジェストを同期しました。構造テストでも新しい境界を検証します。

## 非アクティブなオーバーレイ操作を非表示に維持

ミーティングロビーは、モジュール画面内で HTML の非表示状態を確実に適用するようになりました。これにより、ホストのボタン表示規則が、対応するミーティング状態になる前に認証、セッション再取得、退出、残留の操作を表示することはありません。リリースバージョンと整合性ダイジェストを 1.5.210 に同期しました。
...
Whiteboard の可用性リクエストとレスポンスは、ブラウザーキャッシュを明示的に使用しなくなりました。プロバイダーを有効化または再登録した後に、Meetings が以前の利用不可結果を再利用してコントロール領域を空のままにすることはありません。
