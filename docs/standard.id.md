...
- Papan Tulis yang dibuka meminta kontrak jendela komponen inti `borderless` dan tampilan tanpa bingkai. Cognis core kini menerapkan status `component-page-stage--borderless` yang sesuai sepanjang siklus hidup handle komponen; Meetings hanya melonggarkan tinggi panggung tetap dan overflow terpotong sebagai respons terhadap status milik broker tersebut.
...
- Meetings mendeklarasikan page-builder inti sebelum stylesheet modulnya, memperoleh utilitas browser yang dapat digunakan ulang melalui kapabilitas wajib `ui:reuse`, dan memuat `page-sections.css` melalui permukaan sumber daya tervalidasi tersebut. Core memiliki warna tombol, animasi, status stage komponen, dan deduplikasi stylesheet yang dapat digunakan ulang; CSS modul dibatasi pada geometri khusus Jitsi dan interaksi nonaktif pada tautan.
- Modul mempertahankan `ensureStylesheetLoaded` sebagai ekspor helper lokal yang stabil, tetapi mendelegasikan implementasinya ke modul `page-styles.js` yang diperoleh melalui `ui:reuse`. Hal ini menjaga grafik dependensi SPA tetap valid ketika entry rute dan modul helper diperbarui pada waktu yang berbeda.
