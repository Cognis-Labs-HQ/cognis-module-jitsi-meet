...
- Kata sandi dibuat per rekaman rapat. Rekaman baru hanya menyimpan URL tertunda unik dengan fragmen sampai penyelenggara membuka alur sambutan Jitsi yang didukung; `GENERATE_ROOMNAMES_ON_WELCOME_PAGE` kemudian meminta nama ruang acak milik Jitsi. Modul menangkap ruang yang dilaporkan `videoConferenceJoined`, memvalidasi dan menyimpannya sebagai nama serta URL rapat kanonis, lalu meneruskan nama yang persis sama ke daftar rapat, chat Messages, berbagi, dan Papan Tulis. Rapat tanpa peserta menerima chat Messages beranggota tunggal setelah identitas ditangkap agar tamu tautan berbagi dapat berpartisipasi kemudian; chat tersebut dihapus permanen ketika rapat sekali pakai berakhir. Mulai skrip dan iframe dilindungi penanganan kegagalan serta batas waktu pemuatan iframe. Endpoint konfigurasi `DELETE` yang diautentikasi tetap tersedia saat modul dinonaktifkan agar administrator dapat menghapus URL Jitsi yang tidak valid.
