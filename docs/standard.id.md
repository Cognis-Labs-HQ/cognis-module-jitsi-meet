...
- Kontrol Papan Tulis adalah `<button>` standar yang sama seperti kontrol Bagikan di sebelahnya. Kontrol memakai tampilan core `btn-neutral` secara bawaan serta penanda `active` yang sama seperti item navigasi terpilih bersama `btn-confirm` saat aktif. Memilih kontrol aktif akan menutupnya untuk rapat. Jika persiapan atau pemasangan gagal, Meetings mencatat kegagalan, menampilkan “Terjadi kesalahan saat memuat papan tulis”, dan menonaktifkan kontrol untuk mount browser tersebut agar polling tidak terus mencoba. Memuat ulang atau bernavigasi pergi lalu kembali membuat mount baru dan mengizinkan percobaan lain.
