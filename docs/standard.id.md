...
- Integrasi Papan Tulis opsional muncul ketika pabrik kanvas dasar `whiteboard:uiGateway`, halaman komponen, dan jendela mengambang tersedia; metode opsional untuk kanvas persisten dan penyimpanan peserta meningkatkan integrasi tanpa mengendalikan visibilitas tombol. Hanya rapat tanpa peserta yang memakai kanvas sekali pakai. Rapat dengan peserta undangan memakai kanvas sumber daya persisten, menyimpan pemetaannya untuk setiap peserta, dan membuka kanvas yang dipetakan secara otomatis saat rapat dimuat.
...
- Sebelum komponen Papan Tulis dibuka, Meetings meminta akses gantungan kunci di halaman induk agar tantangan buka kunci memiliki host popup. Papan Tulis kemudian dibuka melalui broker halaman komponen sebagai komponen overlay tertanam dengan pengguliran milik dokumen. Meetings membatasi tampilan tanpa bingkai pada stage Papan Tulisnya dan menghapus status lokal itu secara sinkron saat komponen ditutup, sehingga navigasi SPA tidak dapat meninggalkan shell halaman bersama dalam keadaan tanpa bingkai.
- Kontrol Papan Tulis adalah `<button>` standar yang sama seperti kontrol Bagikan di sebelahnya. Kontrol memakai tampilan core `btn-neutral` secara bawaan serta penanda `active` berlatar aksen yang sama seperti item navigasi terpilih bersama `btn-confirm` saat aktif, dan labelnya berubah menjadi “Tutup Papan Tulis”. Memilih kontrol aktif akan menutupnya untuk rapat. Jika persiapan atau pemasangan gagal, Meetings mencatat kegagalan, menampilkan “Terjadi kesalahan saat memuat papan tulis”, dan menonaktifkan kontrol untuk mount browser tersebut agar polling tidak terus mencoba. Memuat ulang atau bernavigasi pergi lalu kembali membuat mount baru dan mengizinkan percobaan lain.
