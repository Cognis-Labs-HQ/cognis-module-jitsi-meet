...
- The spawned Whiteboard requests the core component-window `borderless` contract and frameless rendering. While core applies `app-page__main--component-borderless` to reduce outer and inner margins, the meeting stage switches from fixed height and clipped overflow to content-driven height so the complete canvas can expand the page vertically.
