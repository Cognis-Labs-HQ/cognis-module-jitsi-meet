...
- Meeting passwords are generated per meeting record. Display names are four-word passphrases generated through the host `reuse:generatePassphrase` capability without a product prefix. New records keep a unique fragment-qualified pending URL until the organizer opens Jitsi’s supported welcome flow, where `GENERATE_ROOMNAMES_ON_WELCOME_PAGE` requests a separate Jitsi room identifier. The module captures and validates the room reported by `videoConferenceJoined` while preserving the passphrase as the name propagated to meeting lists, Messages chats, shares, and Whiteboards. Participant-free meetings receive a single-member Messages chat after identity capture so later share-link guests can participate, and that chat is permanently deleted when the disposable meeting ends. Script and iframe startup are guarded by failure handling and an iframe-load timeout. The authenticated configuration `DELETE` endpoint remains available while the module is disabled so administrators can clear an invalid Jitsi URL.
