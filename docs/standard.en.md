...
- Meeting passwords are generated per meeting record. New records keep a unique fragment-qualified pending URL only until the organizer opens Jitsi’s supported welcome flow, where `GENERATE_ROOMNAMES_ON_WELCOME_PAGE` requests Jitsi’s own random room name. The module captures the room reported by `videoConferenceJoined`, validates and persists it as the canonical meeting name and URL, and propagates that exact name to meeting lists, Messages chats, shares, and Whiteboards. Participant-free meetings receive a single-member Messages chat after identity capture so later share-link guests can participate, and that chat is permanently deleted when the disposable meeting ends. Script and iframe startup are guarded by failure handling and an iframe-load timeout. The authenticated configuration `DELETE` endpoint remains available while the module is disabled so administrators can clear an invalid Jitsi URL.
