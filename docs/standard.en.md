...
- Meeting passwords and opaque Jitsi room identifiers are generated per meeting record with Node Crypto. Every meeting is created with a concrete, unique Jitsi URL before the iframe API is invoked; the iframe API is never opened with an empty room name. Every stored meeting name combines its scheduled UTC minute and a short unique identifier; the same distinguishable name is used by meeting lists, Messages chats, shares, and Whiteboards. Participant-free meetings create single-member Messages chats so later share-link guests can participate, and that chat is permanently deleted when the disposable meeting ends. The authenticated configuration `DELETE` endpoint remains available while the module is disabled so administrators can clear an invalid Jitsi URL.
