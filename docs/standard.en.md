...
- The Whiteboard control is a standard `<button>` matching the adjacent Share control. It uses core `btn-neutral` presentation by default and the imported `active` and `btn-confirm` states while active, and its label changes to “Close Whiteboard”. Selecting the active control closes it for the meeting. Provider, component-page, and keyring capabilities are refreshed and retried while their modules finish loading, including after direct meeting navigation or a page refresh. If preparation or mounting still fails, Meetings logs the failure, displays “Error loading whiteboard,” and keeps a prepared control available for a deliberate retry.
