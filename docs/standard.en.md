...
The server capability requirements explicitly include `whiteboard:fetchBoardData`, `whiteboard:membership`, and `whiteboard:deleteCanvas`. Cognis therefore validates and orders the enabled Whiteboard provider before Jitsi handles state synchronization, instead of allowing the button to reach a route whose scoped context cannot resolve its provider.
