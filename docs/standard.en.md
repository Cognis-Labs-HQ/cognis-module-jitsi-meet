...
- The Whiteboard control is an `<a>` element that uses core `btn-neutral` styling by default and core `btn-confirm` styling while hovered or active. Selecting the active control closes it for the meeting. If preparation or mounting fails, Meetings logs the failure, displays “Error loading whiteboard,” and disables the control for that browser mount so polling cannot repeatedly retry it. Refreshing or navigating away and back creates a new mount and permits another attempt.
