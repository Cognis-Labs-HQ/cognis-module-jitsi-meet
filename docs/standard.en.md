...
- The Whiteboard control is a standard `<button>` matching the adjacent Share control. It uses core `btn-neutral` presentation by default and `btn-confirm` while active. Selecting the active control closes it for the meeting. If preparation or mounting fails, Meetings logs the failure, displays “Error loading whiteboard,” and disables the control for that browser mount so polling cannot repeatedly retry it. Refreshing or navigating away and back creates a new mount and permits another attempt.
