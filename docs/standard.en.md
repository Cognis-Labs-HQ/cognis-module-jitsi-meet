...
- The spawned Whiteboard requests the core component-window `borderless` contract and frameless rendering. While that component handle is active, Meetings applies the matching `component-page-stage--borderless` state to relax the meeting stage’s fixed fill height and clipped overflow, allowing the borderless component window to determine the stage height.
...
- Meetings supplies the anchor control’s complete module-owned layout, typography, hover, active, and disabled styling instead of depending on host button-element defaults or admin-only button classes.
