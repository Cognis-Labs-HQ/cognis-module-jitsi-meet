...
- Meeting passwords are generated per meeting record. Before capture, each record stores a unique fragment-qualified instance URL so multiple new meetings cannot collide on the database URL constraint. When a new meeting has no captured room identity, Meetings disables the embedded Jitsi welcome page so Jitsi immediately generates and joins its default random room; Cognis then captures that name for the Whiteboard resource and undated Messages chat. Other meeting surfaces continue to use “Cognis Classroom” when the room identity is not useful. Participant-free disposable meetings do not create Messages chats, and any previously associated chat is permanently deleted when the meeting ends. The authenticated configuration `DELETE` endpoint remains available while the module is disabled so administrators can clear an invalid Jitsi URL.
