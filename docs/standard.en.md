...
A module-owned backend availability endpoint is the single visibility decision for every account client; browser capability discovery only initializes an already-approved control. The client retries that decision with bounded exponential backoff so a provider completing its enablement registration during the Meetings mount does not permanently leave the Whiteboard control empty. Jitsi declares the optional `whiteboard:uiGateway` requirement so Cognis loads Nextcloud Whiteboard’s dedicated capability provider before mounting the Meetings route when that provider is enabled.
