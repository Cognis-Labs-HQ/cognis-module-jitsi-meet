...
- Meetings declares the core page-builder and reusable page-section styles before its module stylesheet and imports the host `ensurePageStylesheet` utility for optional provider styles. Core now owns reusable button colors, animation, component-stage state, and stylesheet deduplication; module CSS is limited to the anchor’s Jitsi-specific geometry and disabled interaction.
