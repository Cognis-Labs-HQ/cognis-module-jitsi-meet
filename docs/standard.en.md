...
Cognis owns the temporary component stage and its cleanup. Jitsi does not alter the Messages DOM or invoke the component-page broker itself. VoIP consumers may supply a meeting subject; otherwise the provider uses the localized subject “VoIP call.” Component calls omit a module-owned back button, and create no meeting chat. They stay out of the Meetings page’s active and previous meeting lists and cannot be shared, extended with participants, or connected to a Whiteboard.
