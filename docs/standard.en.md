...
Jitsi discovers the optional `whiteboard:uiGateway` only after Cognis has initialized the browser context and mounted the Meetings route. The gateway is deliberately not a route-level capability requirement, preventing Cognis from importing the optional Whiteboard provider before the host UI context exists during a direct page load or refresh. The browser gateway remains the control’s visibility and canvas-factory contract.

The Meetings browser entry consumes the UI context already initialized by the Cognis shell and resolves browser utilities through `ui:reuse`; it does not import the host context implementation.
...
The optional integration is exposed when the base `whiteboard:uiGateway` canvas factory, component-page, and floating-window capabilities are available; the meeting API resolves the provider-declared `whiteboard:fetchBoardData`, `whiteboard:membership`, and `whiteboard:deleteCanvas` contracts, reads the mapped canvas’s actual owner, and invokes the owner-authorized membership `add` and `remove` functions with canonical account IDs before committing participant changes; the persistent `createCanvas` method from the provider contract creates normal canvases with the invited participant handles, while only participant-free meetings use `createDisposableCanvas`.
