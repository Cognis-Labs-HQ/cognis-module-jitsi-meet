...
- Das eingebundene Whiteboard fordert den Core-Komponentenfenstervertrag `borderless` und eine rahmenlose Darstellung an. Während Core mit `app-page__main--component-borderless` äußere und innere Abstände reduziert, wechselt die Besprechungsbühne von fester Höhe und abgeschnittenem Überlauf zu einer inhaltsgesteuerten Höhe, damit die vollständige Zeichenfläche die Seite vertikal erweitern kann.
