...
- Kennwörter und undurchsichtige Jitsi-Raumkennungen werden mit Node Crypto pro Besprechungsdatensatz erzeugt. Jede Besprechung erhält vor dem Aufruf der Iframe-API eine konkrete, eindeutige Jitsi-URL; die Iframe-API wird niemals mit einem leeren Raumnamen geöffnet. Benutzeroberflächen und Gruppenchats verwenden „Cognis Classroom“, wenn die Anzeige der undurchsichtigen Raumkennung keinen Nutzen bietet. Teilnehmerlose, verwerfbare Besprechungen erstellen keine Messages-Chats, und ein zuvor zugeordneter Chat wird beim Beenden der Besprechung dauerhaft gelöscht. Der authentifizierte Konfigurationsendpunkt `DELETE` bleibt auch bei deaktiviertem Modul verfügbar, damit Administratoren eine ungültige Jitsi-URL löschen können.
