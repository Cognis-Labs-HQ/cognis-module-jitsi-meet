...
- Das eingebundene Whiteboard fordert den Core-Komponentenfenstervertrag `borderless` und eine rahmenlose Darstellung an. Solange dieses Komponenten-Handle aktiv ist, setzt Meetings den passenden Zustand `component-page-stage--borderless`, lockert damit die feste Füllhöhe und den abgeschnittenen Überlauf der Besprechungsbühne und lässt das rahmenlose Komponentenfenster die Bühnenhöhe bestimmen.
...
- Meetings stellt für das Anker-Steuerelement vollständige moduleigene Layout-, Typografie-, Hover-, Aktiv- und Deaktiviert-Stile bereit, statt von Host-Standards für Schaltflächenelemente oder nur in der Administration geladenen Schaltflächenklassen abzuhängen.
