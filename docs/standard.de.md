...
- Die optionale Whiteboard-Integration erscheint, wenn die grundlegende Canvas-Factory `whiteboard:uiGateway` sowie Komponentenfenster- und Floating-Window-Fähigkeiten verfügbar sind; optionale Methoden für dauerhafte Canvas und Teilnehmerspeicherung erweitern die Integration, ohne die Sichtbarkeit der Schaltfläche zu steuern. Nur teilnehmerlose Besprechungen verwenden eine verwerfbare Arbeitsfläche. Besprechungen mit eingeladenen Teilnehmern verwenden eine dauerhafte Ressourcen-Arbeitsfläche, speichern deren Zuordnung für alle Teilnehmer und öffnen die zugeordnete Arbeitsfläche beim Laden der Besprechung automatisch.
