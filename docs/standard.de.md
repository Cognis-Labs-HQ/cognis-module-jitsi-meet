...
- Die optionale Whiteboard-Integration erscheint, wenn die grundlegende Canvas-Factory `whiteboard:uiGateway` sowie Komponentenfenster- und Floating-Window-Fähigkeiten verfügbar sind; optionale Methoden für dauerhafte Canvas und Teilnehmerspeicherung erweitern die Integration, ohne die Sichtbarkeit der Schaltfläche zu steuern. Nur teilnehmerlose Besprechungen verwenden die verwerfbare Factory. Besprechungen mit eingeladenen Teilnehmern erfordern die dauerhafte `createCanvas`-Factory und greifen niemals auf verwerfbare Erstellung zurück; sie speichern deren Zuordnung und Verwerfbarkeit für alle Teilnehmer, ersetzen unbekannte oder nicht passende ältere Zuordnungen und öffnen beim Laden der Besprechung automatisch die geprüfte dauerhafte Arbeitsfläche.
