...
Serverseitige Whiteboard-Prüfung, Mitgliedschaft und Löschung werden dynamisch über beide öffentlichen, lebenszyklusgebundenen Capability-Zugriffe aufgelöst: die von aktuellen Providern verwendete Registry `ctx.capabilities` und den Modulkontext-Zugriff für öffentlich beigesteuerte Capabilities. Dies entspricht dem Registrierungsvertrag von Nextcloud Whiteboard und hält Whiteboard für die Jitsi-Aktivierung optional.
