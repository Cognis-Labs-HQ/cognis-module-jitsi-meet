...
Jitsi erkennt das optionale `whiteboard:uiGateway` erst, nachdem Cognis den Browserkontext initialisiert und die Meetings-Route eingebunden hat. Das Gateway ist bewusst keine Capability-Anforderung der Route, damit Cognis den optionalen Whiteboard-Provider beim direkten Laden oder Aktualisieren der Seite nicht vor dem Host-UI-Kontext importiert. Das Browser-Gateway bleibt der Vertrag für Sichtbarkeit und Canvas-Erstellung des Steuerelements.

Der Browser-Einstieg von Meetings verwendet den bereits von der Cognis-Shell initialisierten UI-Kontext und löst Browser-Hilfsmittel über `ui:reuse` auf; die Implementierung des Host-Kontexts wird nicht importiert.
...
Die optionale Integration erscheint, wenn die grundlegende Canvas-Factory `whiteboard:uiGateway` sowie Komponentenfenster- und Floating-Window-Fähigkeiten verfügbar sind; die Meeting-API löst die vom Provider deklarierten Verträge `whiteboard:fetchBoardData`, `whiteboard:membership` und `whiteboard:deleteCanvas` sowie den tatsächlichen Eigentümer der zugeordneten Arbeitsfläche auf und ruft vor dem Speichern von Teilnehmeränderungen die eigentümerautorisierten Mitgliedschaftsfunktionen `add` und `remove` mit kanonischen Konto-IDs auf; die Provider-Vertragsmethode `createCanvas` erstellt normale Arbeitsflächen mit den Kennungen der eingeladenen Teilnehmer, während nur teilnehmerlose Besprechungen `createDisposableCanvas` verwenden.
...
Serverseitige Whiteboard-Prüfung, Mitgliedschaft und Löschung verwenden die öffentlichen Leaf-Capabilities, die Nextcloud Whiteboard über den Modulkontext veröffentlicht. Die Canvas-Erstellung im Browser und diese Server-Capabilities sind getrennte Verträge: Eine erfolgreiche Canvas-Erstellung belegt nicht, dass der Server-Provider veröffentlicht wurde. Bereitstellungen mit der aktuellen Integration benötigen den Bootstrap aus Nextcloud Whiteboard PR #30, der `whiteboard:fetchBoardData` und `whiteboard:membership` direkt veröffentlicht. Jitsi greift weder auf die private API-Fassade des Providers zu noch verwaltet es einen parallelen Capability-Resolver; Whiteboard bleibt für die Jitsi-Aktivierung optional. Die Zuordnungsprüfung vergleicht die vom Provider gelieferte kanonische Ersteller-Konto-ID; bei der Kontolöschung wird die kanonische Cognis-Konto-ID vor der Aktualisierung gespeicherter Teilnehmerlisten in den aktuellen Profil-Handle aufgelöst.
