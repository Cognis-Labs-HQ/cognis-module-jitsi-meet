...
Die Server-Capability-Anforderungen enthalten ausdrücklich `whiteboard:fetchBoardData`, `whiteboard:membership` und `whiteboard:deleteCanvas`. Cognis validiert und ordnet damit den aktivierten Whiteboard-Provider, bevor Jitsi die Zustandssynchronisierung verarbeitet, anstatt die Schaltfläche eine Route aufrufen zu lassen, deren begrenzter Kontext den Provider nicht auflösen kann.
