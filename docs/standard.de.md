...
- Kennwörter werden pro Besprechungsdatensatz erzeugt. Wenn eine neue Besprechung noch keine übernommene Raumidentität besitzt, deaktiviert Meetings die eingebettete Jitsi-Willkommensseite, damit Jitsi sofort einen standardmäßigen zufälligen Raum erzeugt und ihm beitritt; Cognis übernimmt diesen Namen anschließend für die Whiteboard-Ressource und den undatierten Messages-Chat. Andere Besprechungsflächen verwenden weiterhin „Cognis Classroom“, wenn die Raumidentität keinen Nutzen bietet. Teilnehmerlose, verwerfbare Besprechungen erstellen keine Messages-Chats, und ein zuvor zugeordneter Chat wird beim Beenden der Besprechung dauerhaft gelöscht. Der authentifizierte Konfigurationsendpunkt `DELETE` bleibt auch bei deaktiviertem Modul verfügbar, damit Administratoren eine ungültige Jitsi-URL löschen können.
...
- Die Einstieg-URL der Meetings-SPA enthält die Modulversion, damit der Browser nach umbenannten Importen kein unveränderliches Einstiegsmodul einer früheren Version wiederverwendet. Browser-Werkzeuge und der vollständige Katalog gemeinsamer Stylesheets werden vor der Darstellung der Meetings-Oberfläche über die erforderliche Fähigkeit `ui:reuse` geladen. Cognis Core liefert die Standarddarstellung der Steuerelemente; das Modul-CSS besitzt nur das Jitsi-spezifische Layout.
