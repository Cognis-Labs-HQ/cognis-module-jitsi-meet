...
Ein moduleigener Backend-Verfügbarkeitsendpunkt entscheidet einheitlich über die Sichtbarkeit für alle Konto-Clients; die Capability-Erkennung im Browser initialisiert nur ein bereits genehmigtes Steuerelement. Der Client umgeht Browser-Caches und wiederholt diese Entscheidung mit begrenztem exponentiellem Backoff, damit ein Provider, der seine Aktivierungsregistrierung während des Meetings-Mounts abschließt, das Whiteboard-Steuerelement nicht dauerhaft leer lässt. Jitsi deklariert die optionale Anforderung `whiteboard:uiGateway`, damit Cognis den dedizierten Capability-Provider von Nextcloud Whiteboard vor der Meetings-Route lädt, wenn dieser Provider aktiviert ist.
